...
export default function Shop() {
    ...
    // Seuil de Rebirth : 1 dd = 1e39
    const REBIRTH_REQ = 1e39;
    const canRebirth = store.score >= REBIRTH_REQ;
    const nextPriceMult = store.rebirthCount + 3; // Si 0 -> x3, Si 1 -> x4...
        ...
        <div className="page-full bg-shop">
            <div className="game-card" style={{ maxWidth: '900px', width: '95%' }}>
                ...
                <div style={{ marginBottom: '30px' }}>
                    ...
                    {store.score >= store.clickNonillionThreshold && renderUpgradeRow("💫 Nonillion (1No)", store.clickNonillionCost, store.buyNonillion, store.sellNonillion, store.score < store.clickNonillionCost, store.clickNonillionCost <= 1e30, { borderColor: '#74b9ff', background: 'rgba(116, 185, 255, 0.1)' })}
                    {store.score >= store.clickDuodecillionThreshold && renderUpgradeRow("🌀 Duodécillion (1Dd)", store.clickDuodecillionCost, store.buyDuodecillion, store.sellDuodecillion, store.score < store.clickDuodecillionCost, store.clickDuodecillionCost <= 1e39, { borderColor: '#00b894', background: 'rgba(0, 184, 148, 0.1)' })}
                ...
                <div style={{ marginBottom: '30px' }}>
                    ...
                    {store.score >= store.autoNonillionThreshold && renderUpgradeRow("💫 Nonillion Auto", store.autoNonillionCost, store.buyAutoNonillion, store.sellAutoNonillion, store.score < store.autoNonillionCost, store.autoNonillionCost <= 1e30, { borderColor: '#74b9ff', background: 'rgba(116, 185, 255, 0.1)' })}
                    {store.score >= store.autoDuodecillionThreshold && renderUpgradeRow("🌀 Duodécillion Auto", store.autoDuodecillionCost, store.buyAutoDuodecillion, store.sellAutoDuodecillion, store.score < store.autoDuodecillionCost, store.autoDuodecillionCost <= 1e39, { borderColor: '#00b894', background: 'rgba(0, 184, 148, 0.1)' })}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    {!store.catBought ? (<button className="upgrade-btn" onClick={store.buyCatUpgrade} disabled={store.score < store.catUpgradeCost}><span>🐱 Chat (+10/s)</span> <span>{formatNumber(store.catUpgradeCost)} pts</span></button>) : <div className="upgrade-btn" style={{opacity: 0.7, justifyContent: 'center', background: '#e0e0e0'}}>✅ Chat acquis</div>}
                    {!store.cat2Bought ? (<button className="upgrade-btn" onClick={store.buyCat2Upgrade} disabled={store.score < store.cat2UpgradeCost}><span>😼 Chat Ninja (+50/s)</span> <span>{formatNumber(store.cat2UpgradeCost)} pts</span></button>) : <div className="upgrade-btn" style={{opacity: 0.7, justifyContent: 'center', background: '#e0e0e0'}}>✅ Chat Ninja acquis</div>}
                    {!store.cat3Bought ? (<button className="upgrade-btn" onClick={store.buyCat3Upgrade} disabled={store.score < store.cat3UpgradeCost}><span>👑 Roi Chat (+10k/s)</span> <span>{formatNumber(store.cat3UpgradeCost)} pts</span></button>) : <div className="upgrade-btn" style={{opacity: 0.7, justifyContent: 'center', background: '#e0e0e0'}}>✅ Roi Chat acquis</div>}
                    {!store.gooseBought ? (<button className="upgrade-btn" onClick={store.buyGoose} disabled={store.score < store.gooseCost}><span>🪿 L'Oie (+500/s)</span> <span>{formatNumber(store.gooseCost)} pts</span></button>) : <div className="upgrade-btn" style={{opacity: 0.7, justifyContent: 'center', background: '#e0e0e0'}}>✅ Oie acquise</div>}
                    {!store.volcanBought ? (<button className="upgrade-btn" onClick={store.buyVolcan} disabled={store.score < store.volcanCost}><span>Chat Tueur 🔫 (+30/s)</span> <span>{formatNumber(store.volcanCost)} pts</span></button>) : <div className="upgrade-btn" style={{opacity: 0.7, justifyContent: 'center', background: '#e0e0e0'}}>✅ Chat Tueur acquis</div>}
                </div>

                {/* === ZONE ASCENSION (REBIRTH) === */}
                <div style={{ marginTop: '50px', padding: '25px', border: '5px double gold', borderRadius: '15px', background: 'linear-gradient(135deg, #1a1a1a, #000)', boxShadow: '0 0 20px rgba(255, 215, 0, 0.3)' }}>
                    ...
                    {store.rebirthCount < 6 ? (
                        <button
                            ...
                            onClick={store.triggerRebirth}
                            disabled={!canRebirth}
                            style={{
                                width: '100%',
                                background: canRebirth ? 'linear-gradient(45deg, gold, #f1c40f)' : '#333',
                                color: canRebirth ? 'black' : '#666',
                                fontWeight: '900', fontSize: '1.2rem', justifyContent: 'center', height: '60px',
                                border: canRebirth ? '2px solid white' : 'none',
                                cursor: canRebirth ? 'pointer' : 'not-allowed'
                            }}
                        >
                            {canRebirth
                                ? "☄️ DÉCLENCHER LE REBIRTH ☄️"
                                : `Requis : ${formatNumber(REBIRTH_REQ)} (1 Dd)`}
                        </button>
                    ) : (
                        <div style={{ textAlign: 'center', color: 'gold', fontSize: '1.5rem', fontWeight: 'bold', padding: '10px' }}>
